...
def get_dataset(args, task_key, split, tokenizer):
    ...
    #TODO: add dry run sampling
    if args.dry_run:
        df = df.sample(100, random_state=args.seed).reset_index(drop=True)
    # get the data for mtl based on the budget and the upcomming few shot
    # if split == 'train' and:
    #     mtl_task_sample_size = (
    #         args.budget - (args.n_fewshot_tasks * args.k_shot)) // len(args.mtl_tasks.split(","))
    #     df = df.sample(mtl_task_sample_size,
